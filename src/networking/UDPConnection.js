...
class UDPConnection extends EventEmitter {
    constructor(serverIP, serverPort) {
        ...
        Logger.Info("UDPConnection", "constructor", `New UDPConnection constructed. ServerIP: ${serverIP} ServerPort: ${serverPort}`);
    }


    connect() {
        Logger.Info("UDPConnection", "connect", `Connecting to ${this.serverIP}:${this.serverPort}`);
        if (this.isConnected) {
            Logger.Info("UDPConnection", "connect", `Already connected. Disconnecting first.`);
            this.disconnect();
        }
        ...
        this.socket.on('listening', () => {
            Logger.Success("UDPConnection", "connect", "UDP Socket Bound");
            self.emit("connected");
        });
    ...
    attemptReconnect() {
        ...
        Logger.Info("UDPConnection", "attemptReconnect", "Attempting reconnect with delay of : " + this.reconnectTime);
        var self = this;
        setTimeout(() => {
            self.connect();
        }, this.reconnectTime)
    ...
    startListening() {
        ...
        this.socket.on("message", (rawData, info) => {
    ...
    sendPacket(packet) {
        ...
        this.socket.send(stringPacket,this.serverPort, this.serverIP);
    }
